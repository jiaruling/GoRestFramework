...
func QueryFields(m interface{}, fields, exFields []string) (field string, queryField []string) {
	...
	if len(fields) == 0 {
		for i := 0; i < t.Elem().NumField(); i++ {
			...
			re := t.Elem().Field(i).Tag.Get("validate")
...
// 查询条件
func ConditionSQL(r *http.Request, search interface{}) (condition string) {
...
// 排序
func OrderSQL(r *http.Request, order interface{}) (o string) {
	if orders := r.URL.Query().Get("order"); orders != "" {
